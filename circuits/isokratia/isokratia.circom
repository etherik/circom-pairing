...
template commitment(k1, k2) {
    ...
    component hasher = MiMCSponge(3 + k + 6 * 2 * k + 3 * 2 * 2 * k, 220, 1);
    hasher.k <== 123;
...
template Main(levels) {
    ...
    component sigVerify = ECDSAVerifyNoPubkeyCheck(n, k1);
    for (var i = 0;i < k1;i++) {
        sigVerify.r[i] <== r[i];
        sigVerify.s[i] <== s[i];
    ...
    component eligible = VerifyAndModMembership(levels, n, k1);
    eligible.root <== root;
    for (var i = 0;i < k1;i++) {
        for (var j = 0;j < 2;j++) eligible.pubkey[j][i] <== pubkey[j][i];
    }
    for (var i = 0;i < levels;i++) {
        eligible.pathElements[i] <== pathElements[i];
    ...
    component voters = VerifyAndModMembership(levels, n, k1);
    voters.root <== root;
    for (var i = 0;i < k1;i++) {
        for (var j = 0;j < 2;j++) voters.pubkey[j][i] <== pubkey[j][i];
    }
    for (var i = 0;i < levels;i++) {
        voters.pathElements[i] <== pathElements[i];
    ...
    component innerCommitment = commitment(k);
    innerCommitment.ins[0] <== voteCount;
    innerCommitment.ins[1] <== eligibleRoot;
    innerCommitment.ins[2] <== voters.moddedRoot;
    for (var i = 0;i < k1;i++) innerCommitment.ins[i + 3] <== msghash[i];

    // compute inner snark semi-public input commitment
